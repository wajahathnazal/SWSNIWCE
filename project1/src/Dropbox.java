...
public class Dropbox implements Runnable
{
	...
	static String cloud_name = "Dropbox";

	public Dropbox() {
		JDBC.connect_database();

		JFrame jf=new JFrame("Dropbox :: Service provider");
		jf.setLayout(null);
		jf.setVisible(true);
		jf.setSize(450,420);
		...
		ImageIcon icon2= new ImageIcon("images/dropbox.jpg");
	...
	{
		String path = ".//resources//Dropbox//"+data+".txt";
		//path += file_name;
...
private void createFile(String fileName, String fileData) {
	...
	try
	{
		File f = new File(".//resources//Dropbox//"+fileName);
		if(!f.exists())
			f.createNewFile();
