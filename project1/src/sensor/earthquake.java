...
public class earthquake implements ActionListener,Runnable
{
	...
	static private HumanProgressBar progressBar;
	...
	void init()
	{
		...
		temp=new JLabel(new ImageIcon("images/ear1.jpg"),JLabel.LEFT);
		temp.setBounds(5,5,465,310);
		jp2.add(temp);


		jl2=new JLabel(new ImageIcon(""));
		jl3=new JLabel(new ImageIcon("images/earthqk.jpg"));
...
//			out.writeUTF("Earth quake sensor");
//			out.close();
//			client.close();
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
		Thread stat = new Thread() 
		{
			...
			try 
			{
				...
				System.out.println("earthquake sensor server started in "+server.getLocalPort());
				while (true)
				{
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource()==jb)
		{
			if(status>20)
			{
				...
				temp=new JLabel(new ImageIcon("images/earthqk.jpg"),JLabel.LEFT);

				temp.setBounds(15,15,600,300);
				jp2.add(temp);

				jt.setText("earthquake");
				...
				jt1.append("EarthQuake Occured Place is "+place +"\n");    
				jt1.append("No of People Injured "+ t +"\n");	

				if(status >= bomb.status && status >= fire.status)
				{
					JOptionPane.showMessageDialog(null,"earthquake sensor itself is  cluter head \n forwarding data to data center  " + place,"Place",JOptionPane.PLAIN_MESSAGE);
					String time = util.get_time();
					try{
						String earthquakesensorinfo = "";
						
							earthquakesensorinfo="earthquake"+","+"place"+","+place+","+t;
						
						send_data_to_bs("earthquake sensor",earthquakesensorinfo);
						//JOptionPane.showMessageDialog(null,"EarthQuake ocurred in Place  " + place,"Place",JOptionPane.PLAIN_MESSAGE);
					}
					catch(Exception e)
					{
						e.printStackTrace();
				...
				else if(bomb.status >=status  && bomb.status >= fire.status)
				{
					...
					try{
						...
						DataOutputStream don=new DataOutputStream(soc1.getOutputStream());
						don.writeUTF("earthquake sensor");
						System.out.println(earthquakesensorinfo);
						don.writeUTF(earthquakesensorinfo);
						//JOptionPane.showMessageDialog(null,"EarthQuake ocurred in Place  " + place,"Place",JOptionPane.PLAIN_MESSAGE);
					}
					catch(Exception e)
					{
